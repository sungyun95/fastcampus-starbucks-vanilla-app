...
// window 객체, scrollY
// Lodash throttle : 일정 시간마다 함수가 실행되도록 브레이크, 스크롤 할때 많이 사용
window.addEventListener(
  ...
  _.throttle(function () {
    ...
    if (window.scrollY > 500) {
      ...
      // 버튼 숨기기!, id값만 적어줘도 알아서 요소를 찾음, 요소 찾으면 중복은 xx
      gsap.to(toTopEl, 0.2, {
        x: 100,
      });
    }
...
fadeEls.forEach(function (fadeEl, index) {
  gsap.to(fadeEl, 1, {
    delay: (index + 1) * 0.7, // 0.7 1.4 2.1 2.7초 뒤에 발생
    opacity: 1,
  });
});
...
function floatingObject(selector, delay, size) {
  ...
  gsap.to(
    ...
    random(1.5, 2.5), // 애니메이션 동작 시간
    {
      ...
      ease: Power1.easeInOut, // ease
      delay: random(0, delay), // x초 뒤에 애니메이션 시작
    }
  );
}
...
spyEls.forEach(function (spyEl) {
  new ScrollMagic.Scene({
    ...
    triggerHook: 0.8, // 뷰포트 맨위 0, 맨 아래 1
  })
